...
├── dist-server/
│   └── server.js                # Bundle autossuficiente compilado via esbuild
└── src/
    ├── server/
...
| **`EventRepository`** | `users/{userId}/events` | `FamilyEventSchema` | Registros e lembretes de eventos de rotina integráveis com Google Calendar. |
| **`EventMonitorConfigRepository`** | `users/{userId}/monitor_config` | `EventMonitorConfigSchema` | Preferências do monitoramento inteligente de novos eventos externos no Brasil. |
...
*   **Vantagem Crítica**: Ao concatenar módulos locais no bundle síncrono e utilizar resoluções nativas de ES Modules, anulam-se dependências de tempo de execução, otimizando caminhos no container e maximizando a velocidade de boot na porta dedicada (`3000`).
...
## 🛠️ 9. Barramento de Inteligência e Function Calling (AI Tools)

Para permitir que a Aimee interaja proativamente com a vida do usuário de forma útil e estruturada, o sistema conta com um **Barramento de Ferramentas** centralizado no backend, definido no manifesto de IA `/src/server/tools/AimeeTools.ts`.

### A. Fluxo de Execução da Tool
Quando um prompt do usuário é enviado à `/api/ai`, o `AimeeOrchestrator` fornece o arranjo de definições canônicas de ferramentas (`allAimeeTools`) ao modelo Gemini (ou outro adapter polimórfico). Se a IA interpreta que o usuário expressou uma intenção transacional, ela opta por uma chamada estruturada de função (Function Call), que é interceptada pelo orquestrador e processada no backend acionando o repositório adequado.

```mermaid
sequenceDiagram
    participant User as Usuário (Chat React)
    ...
    Orch->>LLM: Envia Prompt + Definições em 'allAimeeTools'
    LLM-->>Orch: Retorna requisição de Function Calling 'addTransaction({amount: 300, type: "income", description: "Venda da mesa"})'
    Orch->>Tools: Executa chamada de persistência síncrona
...
Todas as declarações usam classes de tipo estrito do `@google/genai` (ex: `Type.OBJECT`, `Type.NUMBER`) para assegurar conformidade semântica com o compilador:
...
3.  **`addFinancialGoal` / `updateFinancialGoal`**: Administra objetivos e taxas de poupança no `FinancialGoalSchema`.
4.  **`addHouseholdTask` / `updateHouseholdTask`**: Insere e modifica checklists de tarefas domésticas monitoradas por `TaskRepository`.
5.  **`addFamilyEvent` / `updateFamilyEvent` / `removeFamilyEvent`**: Planeja, ajusta e gerencia compromissos integrados ao Google Calendar através de `EventRepository`.
6.  **`getFinancialInsights`**: Aciona diagnósticos estatísticos preditivos estruturados de economia do `InsightEngine`.
