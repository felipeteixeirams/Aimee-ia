...
import { LogOut, Users, ChevronDown, Sparkles, Zap, Shield } from 'lucide-react';
            ...
            >
              <p className="text-[10px] text-neutral-400 uppercase tracking-widest font-bold whitespace-nowrap">
                {activeSpace ? `Espaço` : 
                 ...
                 profile?.selectedPersona === 'frugal' ? 'Frugal' : 'Online'} • {globalConfig.aiProvider === 'gemini' ? 'Gemini 2.0' : 'DeepSeek R1'}
              </p>
            ...
            <AnimatePresence>
              {showModelDropdown && (
                <motion.div
                  ...
                  className="absolute top-full left-0 mt-2 w-48 bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl border border-neutral-100 dark:border-neutral-800 p-2 z-50 overflow-hidden"
                >
                  <div className="text-[9px] font-black text-neutral-400 uppercase tracking-widest px-3 py-2">Provedor Inteligente</div>
                  <button 
                    onClick={() => { updateGlobalAIProvider('gemini'); setShowModelDropdown(false); }}
                    className={cn(
                      "w-full flex items-center justify-between p-3 rounded-xl transition-all mb-1",
                      globalConfig.aiProvider === 'gemini' ? "bg-brand/10 text-brand" : "hover:bg-neutral-50 dark:hover:bg-neutral-800 text-neutral-500"
                    )}
                  >
                    ...
                    {globalConfig.aiProvider === 'gemini' && <div className="w-1.5 h-1.5 bg-brand rounded-full shadow-[0_0_8px_var(--brand)]" />}
                  </button>
                  <button 
                    onClick={() => { updateGlobalAIProvider('deepseek'); setShowModelDropdown(false); }}
                    className={cn(
                      "w-full flex items-center justify-between p-3 rounded-xl transition-all",
                      globalConfig.aiProvider === 'deepseek' ? "bg-brand/10 text-brand" : "hover:bg-neutral-50 dark:hover:bg-neutral-800 text-neutral-500"
                    )}
                  >
                    <div className="flex items-center gap-3">
                      <Zap className="w-4 h-4" />
                      <span className="text-xs font-bold">DeepSeek R1</span>
                    </div>
                    {globalConfig.aiProvider === 'deepseek' && <div className="w-1.5 h-1.5 bg-brand rounded-full shadow-[0_0_8px_var(--brand)]" />}
                  </button>
                </motion.div>
              )}
            </AnimatePresence>
